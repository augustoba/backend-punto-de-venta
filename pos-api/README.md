...
- **B16** (2026-09-18): servicios (`Product.service`): se venden pero no llevan stock (el movimiento de stock los ignora, no queda negativo ni genera historial). 1 prueba (66 en total).
- **B17** (2026-09-18): recargo en la venta (`discountPct` negativo = recargo, hasta 100 %; el descuento no puede superar 100 %; un recargo no recibe el descuento automático por transferencia). 1 prueba (67 en total).
- **B18** (2026-09-18): devoluciones y cambios (`com.pdv.returns`): devolución parcial o total de una venta, sin poder devolver más de lo vendido; el precio devuelto es el efectivamente pagado (con descuento/recargo de la venta); la mercadería vuelve al stock o se descarta (merma); el dinero se devuelve (efectivo de la caja abierta, transferencia/tarjeta del banco), se acredita en la cuenta corriente del cliente (nota de crédito, sin mover dinero) o queda como cambio; emite nota de crédito de prueba `NC-000001-NNNNNN`. Una venta con devoluciones no se puede eliminar. Endpoints: `GET/POST /api/returns`, `GET /api/sales/{id}/returnable`. 7 pruebas (74 en total).
